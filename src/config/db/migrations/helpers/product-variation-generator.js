...
productVariationData.forEach(row => {
    if (row.table === "product_air_conditioner_variant"){
        ...
        const productVariationvalue = `(${row.fields.pk}, uuid_generate_v4(), '${row.fields.pid}', ${parseFloat(row.fields.retail_price)}, ${parseFloat(row.fields.price)}, ${row.fields.available_stock}, ${row.fields.product_id}, ${row.fields.color_id}, ${row.fields.variant_id}, ${row.fields.active})`
